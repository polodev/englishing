<x-ui-backend::layout>
    <x-slot:title>Create Article Triple Word Set</x-slot:title>
    <div class="container mx-auto px-4 py-6">
        <div class="flex items-center mb-6">
            ...
            <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-200">Create New Article Triple Word Set</h1>
        </div>

        <!-- Form -->
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden">
            <form action="{{ route('backend::article-triple-word-sets.store') }}" method="POST" class="p-6">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="col-span-1 md:col-span-2">
                        ...
                        <input type="text" id="title" name="title" required value="{{ old('title') }}" class="mt-1 block w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                        @error('title')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="col-span-1 md:col-span-2">
                        ...
                        <textarea id="content" name="content" rows="4" class="mt-1 block w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50">{{ old('content') }}</textarea>
                        @error('content')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-1 gap-4">
                                <div>
                                    ...
                                    <textarea id="content_translation_bn" name="content_translation[bn]" rows="3" class="mt-1 block w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50">{{ old('content_translation.bn') }}</textarea>
                                </div>
                                <div>
                                    <label for="content_translation_hi" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Hindi</label>
                                    <textarea id="content_translation_hi" name="content_translation[hi]" rows="3" class="mt-1 block w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50">{{ old('content_translation.hi') }}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="mt-8 flex justify-end">
                    <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                        <i class="fas fa-save mr-2"></i> Save Triple Word Set
                    </button>
                </div>
            </form>
        </div>
