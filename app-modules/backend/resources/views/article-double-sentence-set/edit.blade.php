<x-ui-backend::layout>
    ...
    <div class="container mx-auto px-4 py-6">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden">
            <form action="{{ route('backend::article-double-sentence-sets.update', $articleDoubleSentenceSet) }}" method="POST" class="p-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="col-span-1 md:col-span-2">
                        ...
                        <input type="text" id="title" name="title" required value="{{ old('title', $articleDoubleSentenceSet->title) }}" class="mt-1 block w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                        @error('title')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="col-span-1 md:col-span-2">
                        ...
                        <textarea id="content" name="content" rows="4" class="mt-1 block w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50">{{ old('content', $articleDoubleSentenceSet->content) }}</textarea>
                        @error('content')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="col-span-1">
                        ...
                        <input type="number" id="display_order" name="display_order" value="{{ old('display_order', $articleDoubleSentenceSet->display_order) }}" class="mt-1 block w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                        @error('display_order')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                        ...
                        <div class="mb-4">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="text" id="title_translation_bn" name="title_translation[bn]" value="{{ old('title_translation.bn', $articleDoubleSentenceSet->getTranslation('title_translation', 'bn', false)) }}" class="mt-1 block w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                                </div>
                                <div>
                                    <label for="title_translation_hi" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Hindi</label>
                                    <input type="text" id="title_translation_hi" name="title_translation[hi]" value="{{ old('title_translation.hi', $articleDoubleSentenceSet->getTranslation('title_translation', 'hi', false)) }}" class="mt-1 block w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-1 gap-4">
                                <div>
                                    ...
                                    <textarea id="content_translation_bn" name="content_translation[bn]" rows="3" class="mt-1 block w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50">{{ old('content_translation.bn', $articleDoubleSentenceSet->getTranslation('content_translation', 'bn', false)) }}</textarea>
                                </div>
                                <div>
                                    <label for="content_translation_hi" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Hindi</label>
                                    <textarea id="content_translation_hi" name="content_translation[hi]" rows="3" class="mt-1 block w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50">{{ old('content_translation.hi', $articleDoubleSentenceSet->getTranslation('content_translation', 'hi', false)) }}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="mt-8 flex justify-end">
                    <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                        <i class="fas fa-save mr-2"></i> Update Double Sentence Set
                    </button>
                </div>
            </form>
        </div>
        ...
        <livewire:article-double-sentence--create-article-double-sentence-set :articleDoubleSentenceSet="$articleDoubleSentenceSet" />
    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            @if($articleDoubleSentenceSet->article_id || old('article_id'))
            $.ajax({
                url: '{{ route("backend::api.articles.search") }}',
                data: {
                    q: '{{ old('article_id', $articleDoubleSentenceSet->article_id) }}',
                    exact_id: true
                },
                dataType: 'json'
            }).then(function(data) {
