...
new class extends Component {
    ...
    public function mount($articleId = null)
    {
        ...
        if ($this->articleId) {
            ...
            $lastSet = ArticleDoubleWordSet::where('article_id', $this->articleId)
    ...
    // Create a new article double word set
    public function createArticleDoubleWordSet()
    {
        if (!$this->articleId) {
            session()->flash('error', 'No article selected. Please select an article first.');
            return;
        ...
        // Create the article double word set
        $set = ArticleDoubleWordSet::create([
        ...
        $this->dispatch('article-double-word-set-created', ['articleId' => $this->articleId]);
    }
        ...
        <div class="mb-6 bg-white dark:bg-gray-800 rounded-lg overflow-hidden shadow-xl transform transition-all sm:w-full sm:max-w-4xl mx-auto">
                        ...
                        <div class="flex items-center">
                            ...
                            <div class="ml-3">
                                <p class="text-sm text-green-700 dark:text-green-200">
                                    Double Word Set "{{ $createdSetTitle }}" has been created successfully!
                                </p>
                            </div>
                        </div>
                    </div>
            ...
            @else
                ...
                <div class="px-6 py-4 bg-gray-100 dark:bg-gray-700 border-b border-gray-200 dark:border-gray-600">
                    <h3 class="text-lg font-medium text-gray-900 dark:text-white">Add New Double Word Set to Article</h3>
                    @if($article)
                        <p class="text-sm text-gray-600 dark:text-gray-400">Article: {{ $article->title }}</p>
                    @endif
                </div>

                <form wire:submit.prevent="createArticleDoubleWordSet" class="p-6 dark:bg-gray-800">
                    <!-- Title Input -->
                    <div class="mb-4">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea id="content" wire:model="content" rows="5" class="mt-1 block w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50" placeholder="Write your double word set content here..."></textarea>
