...
<aside :class="{'w-64': sidebarOpen, 'w-16': !sidebarOpen}"
       class="fixed inset-y-0 left-0 z-40 bg-white shadow-lg sidebar-transition hidden lg:block">
    <div class="flex flex-col h-full">
        <div class="flex items-center p-4 border-b" :class="{'justify-center': !sidebarOpen}">
            <div class="flex items-center" :class="{'justify-center w-full': !sidebarOpen}">
                ...
                @endif
                <span class="text-xl font-semibold" x-show="sidebarOpen">{{ config('app.name', 'Englishing.org') }}</span>
            </div>
        </div>
        <nav class="flex-1 p-4">
            <ul class="space-y-2">
                <li>
                    <a href="{{ route('ui-backend::index') }}" class="flex items-center p-2 text-gray-700 rounded-md hover:bg-gray-100" :class="{'justify-center': !sidebarOpen}">
        ...
        <div class="p-4 border-t" x-show="sidebarOpen">
            ...
            <form method="POST" action="{{ route('logout') }}" class="mt-2">
                @csrf
                <button type="submit" class="flex items-center space-x-2 p-2 hover:bg-gray-100 rounded text-red-500 w-full">
                    <i class="fas fa-sign-out-alt mr-2"></i>
                    <span>Logout</span>
                </button>
            </form>
        </div>
        <div class="p-4 border-t text-center" x-show="!sidebarOpen">
            @if(auth()->check())
                @if(auth()->user()->profile_photo_path)
